...
export default function InvoicePdfDocument({ invoiceDetails }: { invoiceDetails: any }) {
  const {
    company,
    company_settings,
    customer,
      ...
      <Page size="A4" style={styles1.page}>
        <View style={styles.headerRow}>
          <View style={styles.companyCol}>
            ...
            <Text style={styles.subHeadline}>{company_settings?.invoice_subheadline || ''}</Text>
          </View>

          <View style={styles.rightHeaderCol}>
            <Text style={styles.rightHeaderText}>{company_settings?.invoice_text_header_email || ''}</Text>
            <Text style={styles.rightHeaderText}>{company_settings?.invoice_text_header_open || ''}</Text>
        ...
        <View style={styles.invoiceRow}>
          <View style={styles.invoiceToCol}>
            <Text style={{ fontSize: 10, color: '#777', marginBottom: 4 }}>INVOICE TO</Text>
            <Text style={{ fontWeight: '700', fontSize: 12 }}>{customer?.customer_name || ''}</Text>
            <Text style={{ fontSize: 10 }}>
              {customer?.address ? `Billing address: ${customer.address}, ${customer.city || ''}` : ''}
            </Text>
            <Text style={{ fontSize: 10, color: '#666' }}>Email: {customer?.email || ''}</Text>
            <Text style={{ fontSize: 10, color: '#666' }}>Phone: {customer?.phone || ''}</Text>
          </View>
          ...
          <View style={styles.invoiceBoxCol}>
            ...
            <View style={styles.invoiceBox}>
              ...
              >
                ...
                <Text style={styles.invoiceBoxValue}>{formatDate(billing?.billing_date)}</Text>
              </View>
              <View
                style={{
                  flexDirection: 'row',
              ...
              >
                ...
                <Text style={styles.invoiceBoxValue}>{money(billing?.amount)}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        {Object.entries(unique_by_utility || {}).map(([key, items]: any, uIdx: number) => {
          const [utilityName = '', , meterNumber = '', ...addressParts] = String(key).split(';');
          const serviceAddress = addressParts.join(';');
          const utilityDetails = items?.[0] ?? {};

          return (
            <View key={String(key) + uIdx} wrap={false} style={{ marginTop: 8 }}>
              ...
              <Text style={styles.utilityTitle}>
                ...
                {meterNumber ? ` - ${meterNumber}` : ''} {serviceAddress ? ` - ${serviceAddress}` : ''}
              </Text>

              {/* Items table (product_id + amount) */}
              <View style={{ marginTop: 4 }}>
                {items.map((item: any, idx: number) => (
                  <View key={idx} style={[styles.tableRow, idx % 2 === 1 ? styles.rowStrip : {}]}>
                    <Text style={styles.tableCellLeft}>{item?.product_id || ''}</Text>
                    <Text style={styles.tableCellRight}>{money(item?.amount)}</Text>
                  </View>
                ))}
                  ...
                  <View style={styles.detailBox}>
                    <View style={styles.detailRow}>
                      <View style={styles.detailCol1}>
                        <Text style={styles.detailSubtitle}>{utilityName} Service at</Text>
                        <Text style={styles.detailValue}>{utilityDetails?.service_address || ''}</Text>
                        <Text
                          style={{
                            fontSize: 10,
                            fontWeight: '600',
                      ...
                      <View style={styles.detailCol2}>
                        <Text style={styles.detailSubtitle}>Number of Days</Text>
                        <Text style={styles.detailValue}>{utilityDetails?.consumption_days ?? '-'}</Text>
                      </View>

                      <View style={styles.detailCol3}>
                        <Text style={styles.detailSubtitle}>Meter Readings</Text>
                      ...
                      <View style={styles.detailCol4}>
                        <Text style={styles.detailSubtitle}>Usage in Gallons</Text>
                        <Text style={{ fontSize: 10, marginBottom: 4 }}>
                          Meter #: {utilityDetails?.meter_number ?? '-'}
                        </Text>
                        <Text style={{ fontSize: 14, fontWeight: '700' }}>{utilityDetails?.consumption ?? '-'}</Text>
                      </View>
                    </View>
                  </View>
                </View>
        ...
        {Array.isArray(extra_params) &&
          extra_params.map((it: any, i: number) => (
            <View key={i} style={styles.prevBalRow}>
              <Text style={{ fontWeight: '700', color: '#666' }}>PREVIOUS BALANCE</Text>
              <Text style={{ fontWeight: '700', color: '#666' }}>{money(it?.amount)}</Text>
            </View>
          ))}

        {/* Total Due Box */}
        <View style={styles.totalBox}>
          <Text style={styles.totalText}>Total Due: {money(billing?.amount)}</Text>
        </View>

        {/* Autopay do not pay text */}
        {customer?.autopay ? (
          <View>
            <Text style={styles.autopayText}>{invoiceDetails?.autopay_do_not_pay_text || ''}</Text>
          </View>
        ) : null}
        {/* Footer: last payment info + company footer */}
        <View style={styles.footerRow}>
          <Text style={styles.lastPaymentText}>{billing?.last_payment_info || ''}</Text>
          <Text style={styles.lastPaymentText}>{company_settings?.invoice_footer_column_3 || ''}</Text>
        </View>
        {/* Divider */}
        ...
        <View style={styles.paper}>
          ...
          <View style={styles.invoiceNumberBox}>
            <Text style={{ fontSize: 10 }}>
              Invoice#: <Text style={{ fontWeight: '700' }}>{billing?.invoice_number || ''}</Text>
            </Text>
          </View>
          ...
          <View style={{ marginTop: 6 }}>
            <Text style={{ fontSize: 11 }}>
              ...
              <Text style={{ fontWeight: '700' }}>{company?.company_name || ''}</Text>
            </Text>
            ...
            <View style={styles.checkboxRow}>
              ...
              />
              <Text style={{ fontSize: 11 }}>
                I would like to go paperless. Here is my email address: ________________________________________
              </Text>
            </View>
          </View>
          ...
          <View style={styles.paySplitRow}>
            ...
            <View style={styles.leftCol}>
              <View>
                <Text style={{ fontWeight: '700' }}>{customer?.customer_name || ''}</Text>
              ...
              <View>
                ...
                <Text style={{ fontSize: 10 }}>
                  {[company?.city, company?.street, company?.zip].filter(Boolean).length > 0 && (
                    <Text>{[company?.city, company?.street, company?.zip].filter(Boolean).join(', ')}</Text>
                  )}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.rightCol}>
              <View style={styles.payTable}>
                ...
                <View style={styles.payRow}>
                  ...
                  <Text style={styles.payCell}>{formatDate(billing?.due_date)}</Text>
                  <Text style={styles.payCellLast}>{money(billing?.amount)}</Text>
                </View>
                ...
                >
                  <Text style={styles.payCell}>{formatDate(billing?.billing_date)}</Text>
                  <Text style={styles.payCell}>{formatDate(billing?.late_date)}</Text>
